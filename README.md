...
```
   ...
   YOU (browser UI)     │            FRONTEND  (React + Vite)          │
                        │   PDF panel · Website panel · Database panel │
   ...
   ────────             │   /api/ingest/website   ─┼─► extract → chunk │
                        ...
                        │                        ┌──────────────────┐  │
                        │                        │  Ollama          │  │
                        │                        │  nomic-embed-text│  │  (turns text → vectors)
                        │                        └────────┬─────────┘  │
                        │                                 ▼            │
                        │                        ┌──────────────────┐  │
                        │                        │  ChromaDB        │  │  (local vector store,
                        │                        │  (on disk)      │  │   persisted to disk)
                        │                        └────────┬─────────┘  │
   CHAT                 │   /api/chat  ──► retrieve ───────┘            │
   ────                 │        │                                     │
                        │        ├─ PDF / website ──► RAG answer       │
                        │        │        (Ollama: llama3.2:3b)        │
                        │        │                                     │
...
1. **Generate SQL.** Send the table card(s) + schema + sample rows + the question
   ...
   *"Write exactly ONE read-only SQL query. Must start with SELECT or WITH. Never
   ...
   All tables from the same database are offered together, so **JOINs** are
   possible.
...
| `POST /api/ingest/database` | `… , tables: [...]` | ingest selected tables (builds card, caches connection) |
...
| `DELETE /api/ingested` | — | wipe everything (also clears cached DB credentials) |
| `DELETE /api/ingested/{source_id}` | — | remove one source (also forgets its credentials) |
...
- **Small local models** make mistakes: SQL for complex multi-JOIN analytics may
  need a retry or come back `INSUFFICIENT`. The trace shows every attempt.
...
ingestion/
  ...
  sources/
    ...
    database_source.py      schema, table card, row chunks, read-only query runner

frontend/                   React + Vite
  src/
    api.js                  typed fetch wrappers for every endpoint
    App.jsx                 layout: 3 ingest panels + chat + "show ingested"
    components/
